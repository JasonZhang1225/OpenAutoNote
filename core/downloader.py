...
def download_video(url: str, output_dir: str = "generate", cookies_yt: str = None, cookies_bili: str = None, strict_output: bool = False, progress_hook=None) -> dict:
    """
    ...
    if "youtube" in url.lower() or "youtu.be" in url.lower():
        if cookies_yt and os.path.exists(cookies_yt):
            cookie_path = cookies_yt
            print(f"[Downloader] Using YouTube cookies: {cookies_yt}")
    elif "bilibili" in url.lower():
        if cookies_bili and os.path.exists(cookies_bili):
            cookie_path = cookies_bili
            print(f"[Downloader] Using Bilibili cookies: {cookies_bili}")
    ...
    if shutil.which('aria2c') and not progress_hook:
        ...
        base_opts['external_downloader_args'] = {'aria2c': ['-x', '16', '-k', '1M', '-s', '16', '--continue=true']}
    elif progress_hook:
